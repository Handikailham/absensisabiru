<nav class="bg-white shadow sticky top-0 z-50">
  <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
    <div class="flex items-center space-x-6">
      ...
      <a href="{{ route('absen.index') }}" class="{{ request()->routeIs('absen.index') ? 'text-blue-600 font-bold' : 'text-gray-700 hover:text-blue-600' }} text-lg">
        Absensi Karyawan
      </a>
      <a href="{{ route('absen.riwayatgaji') }}" class="{{ request()->routeIs('absen.riwayatgaji') ? 'text-blue-600 font-bold' : 'text-gray-700 hover:text-blue-600' }} text-lg">
        Riwayat Gaji
      </a>
      <!-- Dropdown Jadwal Pelatihan -->
      <div x-data="{ open: false }" class="relative">
        <button @click="open = !open" class="flex items-center {{ (request()->routeIs('pelatihankaryawan.index') || request()->routeIs('pelatihankaryawan.requested')) ? 'text-blue-600 font-bold' : 'text-gray-700 hover:text-blue-600' }} text-lg focus:outline-none">
          Jadwal Pelatihan
        ...
        <div x-show="open" @click.away="open = false" class="absolute left-0 mt-2 w-48 bg-white shadow-lg rounded-md z-10">
          <a href="{{ route('pelatihankaryawan.index') }}" class="block px-4 py-2 {{ request()->routeIs('pelatihankaryawan.index') ? 'text-blue-600 font-bold' : 'text-gray-700 hover:bg-blue-500 hover:text-white' }}">
            Pelatihan Terbaru
          </a>
          <a href="{{ route('pelatihankaryawan.requested') }}" class="block px-4 py-2 {{ request()->routeIs('pelatihankaryawan.requested') ? 'text-blue-600 font-bold' : 'text-gray-700 hover:bg-blue-500 hover:text-white' }}">
            Pelatihan Saya
          </a>
        </div>
      </div>
